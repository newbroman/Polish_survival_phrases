...
async function loadLevel(lvl) {
    try {
        ...
        localStorage.setItem('pl_current_level', lvl);
...
function updateMap(filter = "") {
    ...
    if (isSearching) {
        ...
        items = isLearning ? activePool.slice(0, 12) : phrasesData.filter(p => (stats[p.pl] || 0) >= THRESHOLD);
    }
    visibleItems = items; 
...
function nextRound() {
    ...
    if (!visibleItems.length) { qText.innerText = "Done!"; return; }
...
async function startHandsFree() {
    ...
    while (hfActive && !hfAbort && activePool.length > 0) {
        ...
        document.getElementById('hf-current-phrase').innerText = p.pl;
        await speakAsync(p.pl, 0.8, 'pl-PL');
        await sleep(hfDelay / 2);
        
        if (hfAbort) break;
        document.getElementById('hf-current-phrase').innerText = p.en;
...
function skipHandsFree() { window.speechSynthesis.cancel(); }
function resetEverything() { if(confirm("Reset progress?")) { stats = {}; saveStats(); location.reload(); } }

function speakTarget() { if(currentTarget) speak(currentTarget.pl); }
function applyUILang() { /* Logic for translating UI if needed */ }
function toggleUILanguage() { uiLang = (uiLang === 'EN' ? 'PL' : 'EN'); localStorage.setItem('pl_ui_lang', uiLang); location.reload(); }
...
function loadProgressFromFile() {
    ...
    input.onchange = e => {
        ...
        reader.onload = event => {
            stats = JSON.parse(event.target.result);
            saveStats();
            location.reload();
        };
